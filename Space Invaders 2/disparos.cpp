...
bool colicion(int x1,int y1,int w1,int h1,int x2,int y2, int w2,int h2){
    return((x1<x2+w2)&&(x2<x1+w1)&&(y1<y2+h2)&&(y2<y1+h1));//si la condicion es verdad nos regresa verdad y si uno falla es falsa
}
...
void eliminar_bala_escudo(struct NAVE&N,struct escudo ES[],struct Balas B[]){
    if ( N.n_disp > 0 && N.n_disp < N.max_disp){//recorrer el arreglo
        for ( int cont = 1; cont <= N.n_disp; cont++){//recorrer el arreglo
                for(int i=0;i<20;i++){
                    if(colicion(ES[i].x,ES[i].y,20,16,B[cont].x,B[cont].y,N.ancho_b,N.alto_b)
                       &&ES[i].dan<3){
                            eliminar(B,N.n_disp,cont);
                            ES[i].dan++;
                       }
